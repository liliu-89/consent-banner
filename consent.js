(() => {
  ...
  function loadGTM(id) {
    if (!id || id === "GTM-XXXX") {
      console.warn("[Consent] GTM ID fehlt oder ungueltig");
      return;
    }
...
.cb-box.visible{opacity:1}
.cb-inner{padding:1.35rem}
#cb-title{margin:0 0 .5rem; font-size:1.35rem; font-weight:700}
.cb-desc{margin:.35rem 0 1rem; font-size:1rem}
.cb-desc a{color:var(--brand); text-decoration:underline}
.cb-actions{display:flex; flex-direction:column; gap:.6rem; margin:1rem 0}
@media (min-width:560px){ .cb-actions{flex-direction:row} }
.cb-btn{
  display:flex; justify-content:center; align-items:center;
  padding:12px 14px; border-radius:10px;
  font-weight:600; border:1px solid transparent; cursor:pointer;
}
.cb-btn-primary{background:var(--brand); color:#fff; border-color:var(--brand)}
.cb-btn-tonal{background:var(--tone); color:var(--on-tone)}
.cb-btn-outline{border-color:var(--brand); color:var(--brand)}
#cb-settings-toggle{margin:.5rem 0 .75rem 0}
#cb-switches-wrapper{
...
.cb-switches{display:grid; gap:.6rem; margin:.25rem 0 .5rem}
.cb-switch{
  display:flex; align-items:center; gap:.6rem;
  padding:.55rem .8rem; background:#fff;
  border:1px solid var(--surface-variant); border-radius:10px;
}
.cb-switch input{
  width:20px; height:20px; accent-color:var(--brand);
}
.cb-links{text-align:center; margin-top:12px; font-size:.9rem}
  `.trim();

  const HTML = `
<div id="consent-banner" hidden aria-live="polite">
  <div class="cb-overlay"></div>
  <div class="cb-box" role="dialog" aria-modal="true">
    <div class="cb-inner">
      <h2 id="cb-title">Datenschutz Einstellungen</h2>
      <p id="cb-desc" class="cb-desc">
        Wir verwenden Cookies fuer den sicheren Betrieb der Seite.
        Mit Ihrer Zustimmung nutzen wir Cookies fuer Statistiken und Marketing.
        Sie koennen Ihre Auswahl jederzeit aendern.
      </p>
      ...
      <button id="cb-settings-toggle" class="cb-btn cb-btn-tonal">Einstellungen anzeigen</button>
      ...
      <div id="cb-switches-wrapper" hidden>
        <div class="cb-switches">
          ...
          <label class="cb-switch">
            <input type="checkbox" id="toggle-statistics"> Statistiken
          </label>
          <label class="cb-switch">
            <input type="checkbox" id="toggle-marketing"> Marketing
          </label>
          <button id="btn-accept-selection" class="cb-btn cb-btn-tonal">Auswahl speichern</button>
        </div>
      </div>

      <p class="cb-links">
        <a href="${PRIV_URL}" rel="nofollow">Datenschutzerklaerung</a>
...
`.trim();
  ...
  function setCookie(n, v) {
    ...
    document.cookie = encodeURIComponent(n) + "=" + encodeURIComponent(v) + "; expires=" + d.toUTCString() + "; path=/; SameSite=Lax; Secure";
  }
  ...
  function injectBanner() {
    ...
    if (!box) {
      console.warn("[Consent] Banner blockiert – kein Scroll-Lock gesetzt");
    ...
    if (stored) {
      ...
      setTimeout(() => {
        if (!scrollLockApplied) {
          console.warn("[Consent] Fallback – Scroll-Lock deaktiviert");
          document.body.style.overflow = "";
        }
      }, 80);
    }
    ...
    setTimeout(() => {
      ...
      if (
        ...
        document.body.style.overflow = "";
        console.warn("[Consent] Globaler Fallback – scroll-lock entfernt");
      }
    }, 200);
  }
